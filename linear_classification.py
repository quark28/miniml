...
class LinearClassification():
    ...
    '''
    @staticmethod
    def _hinge_loss_func(y_pred, y_true): #1
    ...
    def _logloss_loss_func(y_pred, y_true): #3
        return np.mean(np.log2(1 + np.exp(-(y_true * y_pred))))
    @staticmethod
    ...
    def _ann_loss_func(y_pred, y_true): #5
        return np.mean(2 * (1 + np.exp( y_true * y_pred )) ** -1)
    @staticmethod
    ...
    def __init__(self, loss, regularizator = None, delta = None, alpha = None):
        ...
        if loss == 'hinge':
            ...
            self.loss_func = self._hebb_rule_loss_func
            self.loss_func_derivative = self._hebb_rule_loss_func_derivative
        elif loss == 'logloss':
            self.loss_func = self._logloss_loss_func
            self.loss_func_derivative = self._logloss_loss_func_derivative
        elif loss == 'fld':
            self.loss_func = self._fld_loss_func
            self.loss_func_derivative = self._fld_loss_func_derivative
        elif loss == 'ann':
            self.loss_func = self._ann_loss_func
            self.loss_func_derivative = self._ann_loss_func_derivative
        elif loss == 'adaboost':
            self.loss_func = self._adaboost_loss_func
            self.loss_func_derivative = self._adaboost_loss_func_derivative
    ...
    def predict(self, X):
        ...
        scores = np.sign(X_with_bias @ self.w)
        if self._orig_classes <= {0, 1}:
            return np.where(scores == 0, np.nan, (scores + 1) // 2)
        return np.where(scores == 0, np.nan, scores)