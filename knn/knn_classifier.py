...
class KNNClassification:
    ...
    def __init__(self, neighbours=5, metric='euclid', p=None, weight_type='uniform', q=None, h=1):
        ...
        if metric == 'euclid':
            self.metric = KNNClassification._euclid
        elif metric == 'manhattan':
            self.metric = KNNClassification._manhattan
        elif metric == 'minkowski':
            self.metric = KNNClassification._minkowski_maker(p)
        elif metric == 'cosine':
            self.metric = KNNClassification._cosine
        elif metric == 'jacquard':
            self.metric = KNNClassification._jacquard
